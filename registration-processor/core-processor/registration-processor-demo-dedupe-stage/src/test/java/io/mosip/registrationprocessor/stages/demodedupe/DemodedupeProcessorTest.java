...
public class DemodedupeProcessorTest {
	...
	public void testDemoDedupeNewPacketSuccess() throws Exception {
		...
		Mockito.when(utility.getApplicantAge(anyString(),anyString())).thenReturn(20);
	...
	public void testDemoDedupeUpdatePacketSuccess() throws Exception {
		...
		when(utility.getDefaultSource(anyString(),any())).thenReturn(source);
		...
		Mockito.when(demoDedupe.performDedupe(anyString())).thenReturn(emptyDuplicateDtoSet);
		Mockito.when(packetInfoManager.getIdentityKeysAndFetchValuesFromJSON(any(),any())).thenReturn(individualDemoDedupe);
		MessageDTO messageDto = demodedupeProcessor.process(dto, stageName);
		assertTrue(messageDto.getIsValid());
	}
	...
	public void testDemoDedupeResUpdatePacketSuccess() throws Exception {
		...
		Mockito.when(packetInfoManager.getIdentityKeysAndFetchValuesFromJSON(any(),any())).thenReturn(demographicData);
		MessageDTO messageDto = demodedupeProcessor.process(dto, stageName);
		assertTrue(messageDto.getIsValid());
	}
	...
	@SuppressWarnings("unchecked")
	@Test
	public void testDemoDedupeFailure() throws ApisResourceAccessException, IOException, JsonProcessingException, Exception {
		InternalRegistrationStatusDto registrationStatusDto = new InternalRegistrationStatusDto();
	...
	@SuppressWarnings("unchecked")
	@Test
	public void testResourceException() throws ApisResourceAccessException, IOException, JsonProcessingException, Exception {
		when(env.getProperty(DEMODEDUPEENABLE)).thenReturn("true");
		...
		Mockito.doThrow(exp).when(utility).getApplicantAge(anyString(),anyString());
	...
	public void testFSAdapterExceptionException()
			throws ApisResourceAccessException, IOException, JsonProcessingException, Exception {
		Mockito.when(demoDedupe.performDedupe(anyString())).thenReturn(duplicateDtos);
	...
	@Test
	public void testIllegalArgumentException() throws ApisResourceAccessException, IOException, JsonProcessingException, Exception {
		Mockito.when(demoDedupe.performDedupe(anyString())).thenReturn(duplicateDtos);
	...
	public void testDemoDedupeNewChildPacketSuccess() throws Exception {
		...
		byte[] b = "sds".getBytes();
		List<DemographicInfoDto> emptyDuplicateDtoSet = new ArrayList<>();
		Mockito.when(utility.getApplicantAge(anyString(),anyString())).thenReturn(2);
		PowerMockito.mockStatic(JsonUtil.class);
		PowerMockito.mockStatic(IOUtils.class);
		PowerMockito.when(JsonUtil.class, "inputStreamtoJavaObject", inputStream, PacketMetaInfo.class)
				.thenReturn(packetMetaInfo);
	...
	public void testDemoDedupeDisabledSuccess() throws Exception {
		...
		byte[] b = "sds".getBytes();
		List<DemographicInfoDto> emptyDuplicateDtoSet = new ArrayList<>();
		Mockito.when(utility.getApplicantAge(any(),any())).thenReturn(20);
		PowerMockito.mockStatic(JsonUtil.class);
		PowerMockito.mockStatic(IOUtils.class);
		PowerMockito.when(JsonUtil.class, "inputStreamtoJavaObject", inputStream, PacketMetaInfo.class)
				.thenReturn(packetMetaInfo);
